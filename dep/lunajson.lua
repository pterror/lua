...
local function f_tostring(v2)
	builder[i] = tostring(v2)
	i = i + 1
end
...
setmetatable(enc_dispatcher, enc_dispatcher)
...
local decode_error = function (errmsg)
	return error("parse error at " .. pos .. ": " .. errmsg, 2)
end

local f_err = function ()
...
-- FIXME: redefined local `radixmark`

-- deal with non-standard locales
local radixmark2 = match(tostring(0.5), "[^0-9]")
local fixedtonumber = tonumber
if radixmark2 ~= "." then
	if find(radixmark2, "%W") then
		radixmark2 = "%" .. radixmark2
	end
	fixedtonumber = function(s)
		return tonumber(gsub(s, ".", radixmark2))
	end
end

local number_error = function ()
...
local surrogate_first_error = function ()
...
setmetatable(dec_dispatcher, dec_dispatcher)
