...
--[[@class chui_widget]]
--[[@field __index chui_widget]]
--[[@field defaults table]]
--[[@field panel chui_panel]]
--[[@field span integer]]
--[[@field pose lovr_mat4]]
--[[@field interactive? boolean]]
--[[@field init fun(self: chui_widget, options: table)]]
--[[@field draw fun(self: chui_widget, pass: lovr_pass)]]
--[[@field update fun(self: chui_widget, dt: number, pointer: lovr_vec3, handness?: chui_hand)]]
...
--[[@param pose lovr_mat4]]
function m.spacer:draw(pass, pose)
end

--[[@param dt number]]
--[[@param pointer lovr_vec3]]
--[[@param handness? chui_hand]]
function m.spacer:update(dt, pointer, handness)
end

-- LABEL ----------------------------------------------------------------------
--[[@class chui_label: chui_widget]]
m.label = {}
m.label.defaults = { text = "", text_scale = 1 }
table.insert(m.widget_types, "label")

--[[@class chui_label_options]]
--[[@field text? string default=`''`]]
--[[@field text_scale? number default=`1`]]

--[[@param options chui_label_options]]
...
-- BUTTON ---------------------------------------------------------------------
--[[@class chui_button: chui_widget]]
m.button = {}
m.button.defaults = { text = "", thickness = 0.3, callback = nil }
table.insert(m.widget_types, "button")

--[[@class chui_button_options]]
--[[@field text string default=`''`]]
--[[@field thickness number default=`0.3`]]
--[[@field callback fun(self: chui_button) default=`nil`]]

--[[@param options chui_button_options]]
...
-- TOGGLE ---------------------------------------------------------------------
--[[@class chui_toggle: chui_widget]]
--[[@field state boolean]]
m.toggle = {}
m.toggle.defaults = { text = "", thickness = 0.3, state = false, callback = nil }
table.insert(m.widget_types, "toggle")

--[[@class chui_toggle_options]]
--[[@field text? string default=`''`]]
--[[@field thickness? number default=`0.3`]]
--[[@field state? boolean default=`false`]]
--[[@field callback? fun(self: chui_toggle, state: boolean) default=`nil`]]

--[[@param options chui_toggle_options]]
...
-- GLOW -------------------------------------------------------------------------
--[[@class chui_glow: chui_widget]]
m.glow = {}
m.glow.defaults = { text = "", thickness = 0.1, state = false }
table.insert(m.widget_types, "glow")

--[[@class chui_glow_options]]
--[[@field text? string default=`''`]]
--[[@field thickness? number default=`0.1`]]
--[[@field state? boolean default=`false`]]

--[[@param options chui_glow_options]]
...
function m.progress:draw(pass)
	...
	local text_y = 0.2
	pass:setColor(self.panel.palette.text)
	pass:text(self.text, 0, text_y, 2 * Q, text_scale)
end
...
--[[@class chui_slider_options]]
--[[@field text? string default=`''`]]
--[[@field format? string default=`'%s %.2f'`]]
--[[@field min? number default=`0`]]
--[[@field max? number default=`1`]]
--[[@field value? number default=`0`]]
--[[@field step? number default=`nil`]]
--[[@field thickness? number default=`0.15`]]
--[[@field callback? fun(self: chui_slider, value: number) default=`nil`]]

--[[@param options chui_slider_options]]
...
--[[@class chui_panel_options]]
--[[@field frame? string default=`'backpanel'`]]
--[[@field pose? lovr_mat4]]
--[[@field palette? chui_palette]]

--[[@param options? chui_panel_options]]
function m.panel(options)
	...
	self.widgets = {} --[[@type chui_widget[] ]]
	self.rows = { {} } --[[@type chui_widget[][] ]]
	self.width = 0
	self.height = 0
...
function panel:updateWidgets(dt, pointers)
	...
	for _, widget in ipairs(self.widgets) do
		local closest_pos
		local closest_hand --[[@type chui_hand]]
...
--[[@param pointers chui_pointer[] ]]
--[[@param click_offset? boolean]]
function panel:getMousePointer(pointers, click_offset)
	local _, _, _, scale = self.pose:unpack()
	...
	local ray_origin = vec3(self.world_from_screen:mul(vec3(x, y, 1)))
	local ray_target = vec3(self.world_from_screen:mul(vec3(x, y, 0.001)))
...
function panel:draw(pass, draw_pointers)
	...
	if draw_pointers then
		local pointers = self:getPointers(false)
...
--[[@param widget_proto chui_widget]]
...
--[[@param font string|lovr_font path or font instance]]
...
function m.update(dt) -- neccessary for UI interactions
	for _, panel_ in ipairs(m.panels) do
		panel_:update(dt)
	end
end
...
function m.draw(pass, draw_pointers)
	for _, panel_ in ipairs(m.panels) do
		panel_:draw(pass, draw_pointers)
	end
end
