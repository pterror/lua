...
local iskind = function (prop, tag)
...
local isnode = function (prop)
	return iskind(prop, "Node")
end

local kind2str
kind2str = function (spec)
...
local check_node = function (tag, prop)
...
local check_list = function (spec, prop)
	if type(prop) ~= "table" then
...
local check_enum = function (spec, prop)
...
local check_literal = function (spec, prop)
...
local check_choice = function (spec, prop)
	for i = 1, #spec.values do
...
local validate = function (meta, node)
	if meta == nil then
	...
	for name, spec in pairs(meta.properties) do
		...
		if prop ~= nil or not spec.optional then
			local ok, err = check(spec, prop)
			if not ok then
				error(err .. " for " .. (node.kind or "?") .. "." .. name)
			end
		end
	end
	return node
