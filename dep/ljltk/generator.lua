...
local bc = require("dep.ljltk.bytecode")
local const_eval = require("dep.ljltk.ast_const_eval")
local boolean_const_eval = require("dep.ljltk.ast_boolean_const_eval")

local format = string.format

local id = 0
local genid = function () id = id + 1; return "__" .. id end
...
local lang_error = function (msg, chunkname, line)
	error(format("LLT-ERROR%s:%d: %s", chunkname, line, msg), 0)
end
...
-- bitfield operations
local EXPR_RESULT_TRUE = 1
local EXPR_RESULT_FALSE = 2
...
local is_literal = function (node) return node.kind == "Literal" end
local is_identifier = function (node) return node.kind == "Identifier" end
...
local is_vcall = function (node) return MultiExprRule[node.kind] ~= nil end
...
-- if `src` evaluates to `true`/`false` according to `cond`.
local cond_mov_toreg = function (ctx, cond, dest, src, jump_label, jreg)
	if dest ~= src then ctx:op_testmov(cond, dest, src, jump_label, jreg)
	else ctx:op_test(cond, src, jump_label, jreg) end
end

local is_byte_number = function (v) return type(v) == "number" and v % 1 == 0 and v >= 0 and v < 256 end
...
ExpressionRule.Literal = function (self, node, dest) self.ctx:op_load(dest, node.value) end

ExpressionRule.Identifier = function (self, node, dest)
...
ExpressionRule.Vararg = function (self, node, dest) self.ctx:op_varg(dest, 1) end
...
ExpressionRule.Table = function (self, node, dest)
	...
	for k = 1, #node.keyvals do
		...
		if key then
			...
			if k_is_const and kval ~= nil and v_is_const then
				...
				local ktag, kval2 = self:expr_toanyreg_tagged(key, EXPR_EMIT_VSB)
				local v = self:expr_toanyreg(value)
				self.ctx:op_tset(free, ktag, kval2, v)
...
ExpressionRule.ConcatenateExpression = function (self, node, dest)
	local free = self.ctx.freereg
...
ExpressionRule.BinaryExpression = function (self, node, dest, jreg)
	local free = self.ctx.freereg
...
ExpressionRule.ExpressionValue = function (self, node, dest, jreg)
	self:expr_toreg(node.value, dest, jreg)
end

ExpressionRule.UnaryExpression = function (self, node, dest)
	local free = self.ctx.freereg
...
ExpressionRule.LogicalExpression = function (self, node, dest, jreg)
...
ExpressionRule.MemberExpression = function (self, node, dest)
	local free = self.ctx.freereg
...
ExpressionRule.FunctionExpression = function (self, node, dest)
	local free = self.ctx.freereg
...
MultiExprRule.CallExpression = function (self, node, want, tail)
	return emit_call_expression(self, node, want, tail, false)
end

MultiExprRule.SendExpression = function (self, node, want, tail)
	return emit_call_expression(self, node, want, tail, true)
end

LHSExpressionRule.Identifier = function (self, node)
...
LHSExpressionRule.MemberExpression = function (self, node)
...
TestRule.Literal = function (self, node, jmp, jreg, negate, store, dest)
...
TestRule.BinaryExpression = function (self, node, jmp, jreg, negate, store, dest)
	local o = node.operator
	if cmpop[o] then
		local free = self.ctx.freereg
		local atag, a, btag, b
...
TestRule.UnaryExpression = function (self, node, jmp, jreg, negate, store, dest)
...
TestRule.LogicalExpression = function (self, node, jmp, jreg, negate, store, dest)
	local or_operator = node.operator == "or"
	local lstore = bit.band(store, store_bit(or_operator))
	if (negate or false) ~= or_operator then
		local templ = genid()
...
StatementRule.StatementsGroup = function (self, node)
	for i = 1, #node.statements do self:emit(node.statements[i]) end
end

StatementRule.CallExpression = function (self, node) self:expr_tomultireg(node, 0, false) end
StatementRule.SendExpression = function (self, node) self:expr_tomultireg(node, 0, false) end
...
StatementRule.IfStatement = function (self, node, root_exit)
	local free = self.ctx.freereg
...
StatementRule.ExpressionStatement = function (self, node) return self:emit(node.expression) end
...
StatementRule.AssignmentExpression = function (self, node)
	...
	for i=1, nexps - 1 do
		...
		if use_reg then exprs[i] = self:expr_tonextreg(node.right[i])
		else exprs[i] = self:expr_toanyreg(node.right[i]) end
		slots = slots - 1
	end
	...
	if slots == 1 then
		...
		for k = slots - 1, 0, -1 do self:assign(lhs[i + k], exp_base + k) end
	end

	for j = nvars - slots, 1, -1 do self:assign(lhs[j], exprs[j]) end
...
StatementRule.WhileStatement = function (self, node)
	...
	local loop_begin_location = genid()
	local loop_exit_location = genid()
...
StatementRule.Chunk = function (self, node, name)
	self:block_emit(node.body)
	self:close_proto()
end
...
local generate = function (tree, name)
	local this = { line = 0 }
	this.main = bc.Proto.new(bc.Proto.VARARG, tree.firstline, tree.lastline)
	this.ctx = this.main
	this.chunkname = tree.chunkname

	this.block_enter = function (self)
		self.ctx:enter()
	end

	this.block_leave = function (self, lastline, exit)
		self.ctx:fscope_end()
	...
	this.loop_enter = function (self, exit, exit_reg)
		self:block_enter()
		self.ctx:loop_register(exit, exit_reg)
	end

	this.loop_leave = function (self, lastline)
		self:block_leave(lastline)
	end

	this.assign = function (self, lhs, expr)
	...
	this.block_emit = function (self, stmts, if_exit)
		local n = #stmts
		for i = 1, n - 1 do
			self:emit(stmts[i])
		end
	...
	this.test_emit = function (self, node, jmp, jreg, negate, store, dest)
		if node.line then self.ctx:line(node.line) end
	...
	this.expr_test = function (self, node, jmp, jreg, negate, store, dest)
		local free = self.ctx.freereg
		...
		if const_val ~= nil then
			...
			if (negate or false) ~= not const_val then
				self.ctx:jump(jmp, jreg)
			end
		else
	...
	this.expr_toanyreg = function (self, node, tail)
	...
	this.expr_toreg = function (self, node, dest, jreg, tail)
		if node.line then self.ctx:line(node.line) end
	...
	-- store is (dest + 1).
	this.expr_tonextreg = function (self, node)
		local dest = self.ctx.freereg
	...
	this.expr_tomultireg = function (self, node, want, tail)
		if node.line then self.ctx:line(node.line) end
	...
	-- itself.
	this.expr_toanyreg_tagged = function (self, node, emit)
		local const_val = const_eval(node)
	...
	this.property_tagged = function (self, property_name)
	...
	this.expr_tolhs = function (self, lhs, expr)
		local free = self.ctx.freereg
		if lhs.tag == "upval" then
			local tag, expr2 = self:expr_toanyreg_tagged(expr, EXPR_EMIT_VSNP)
			self.ctx:op_uset(lhs.uv, tag, expr2)
	...
	this.lhs_expr_emit = function (self, node)
	...
	this.close_proto = function (self, lastline)
		if lastline then self.ctx:line(lastline) end
	...
	local dump = bc.Dump.new(this.main, name)
	return dump:pack()
end
