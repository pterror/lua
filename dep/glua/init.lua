...
local lib = require("dep.glua.gl.glut")
local ffi = require("ffi")
local bit = require("bit")
local gl = require("dep.glua.matrix") --[[inherit matrix operators]]

--[[OSX compatiblity extensions]]
local onOSX = false
if ffi.os == "OSX" then
	require("dep.glua.mac.glext")
	onOSX = true
end

--[[load image library, use CoreGraphics on Mac]]
--[[and libpng on other platforms]]
local imglib = require(onOSX and "dep.glua.mac.CoreGraphics" or "dep.glua.lib.png")
...
local glClampd = ffi.typeof("GLclampd")
local glClampdv = ffi.typeof("GLclampd[?]")
--[[@diagnostic disable-next-line: assign-type-mismatch]]
local glCharv = ffi.typeof("GLchar[?]") --[[@type fun(length: integer): string_c]]
...
gl.utTimerCallback = function (f) return ffi.cast("glutTimerCallback", f) end
gl.utIdleCallback = function (f)  return ffi.cast("glutIdleCallback", f)  end

--[[PATH UTIL -----------------------------------------------------------------]]

gl.path = function (name, type)
	type = type or ''
...
gl.GetShader = function (shader, what)
...
gl.ShaderSource = function (shader, source)
	...
	lib.glShaderSource(shader, 1, sourcepp, nil)
end
...
gl.GetProgram = function (program, what)
	local class = glGetProgramMap[what]
	if class == nil then return nil end
	local m = glIntv(class[1])
	gl.GetProgramiv(program, what, m)
	if class[1] == 1 then
		if class[2] then return m[0] == gl.TRUE end
		return m[0]
	end
	return m
end

local gl_GetProgramInfoLog = gl.GetProgramInfoLog --[[@type fun(...)]]
gl.GetProgramInfoLog = function (program)
	...
	local log = glCharv(logSize+1)
	gl_GetProgramInfoLog(program, logSize+1, logSizep, log)
	--[[@diagnostic disable-next-line: undefined-global]]
	return ffi.string(constCharp) --[[FIXME:]]
end

local programMT = {}
gl.program = function (shaderPaths)
...
programMT.__index = function (self, uniform)
...
programMT.__newindex = function (self, uniform, value)
...
programMT.__gc = function (self) gl.DeleteProgram(self.gl) end
programMT.__call = function (self) gl.UseProgram(self.gl); return self end

--[[TEXTURES ------------------------------------------------------------------]]

--[[@return integer[] out]]
--[[@param num? integer defaults to 1]] --[[@param out? integer[] ]]
gl.GenTextures = function (num, out)
	num = num or 1
	out = out or glUintv(num)
	lib.glGenTextures(num, out)
	return out
end
gl.GenTexture = function ()
...
gl.texture = function (path, target)
...
textureMT.__gc = function (self) gl.DeleteTexture(self.gl) end
textureMT.__call = function (self, target)
	if target then gl.ActiveTexture(gl.TEXTURE0 + target) end
	gl.BindTexture(self.gl)
	return self
end

gl.textures = function (paths)
	local textures = {}
...
--[[@class gl_uintv_c: integer]]

--[[@return gl_uintv_c[] out]]
--[[@param num integer]] --[[@param out gl_uintv_c[]?]]
gl.GenBuffers = function (num, out)
	num = num or 1
	out = out or glUintv(num)
...
gl.GenVertexArrays = function (num, out)
	num = num or 1
	out = out or glUintv(num)
...
gl.array = function (program, data, ...)
...
gl.draw = function (program, mode, data, ...)
	local glProgram = type(program) == "table" and program.gl or program
	local ptr = glFloatv(#data, data)
	gl.DrawArrays(gl.TRIANGLES, 0, math.floor(#data / glLoadArray(glProgram, ptr, ...)))
end

arrayMT.__gc = function (self) gl.DeleteVertexArray(self.gl) end

--[[@param mode integer defaults to gl.TRIANGLES]] --[[@param from? integer]] --[[@param size? integer]]
arrayMT.__call = function (self, mode, from, size)
