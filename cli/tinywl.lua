...
mod.focus_toplevel = function(toplevel, surface)
	...
	if prev_surface ~= nil then
		...
		local prev_toplevel =
				wlr.xdg_toplevel_try_from_wlr_surface(prev_surface)
		if prev_toplevel ~= nil then
			wlr.xdg_toplevel_set_activated(prev_toplevel, false)
		end
	end
	local keyboard = wlr.seat_get_keyboard(seat)
	--[[Move the toplevel to the front]]
	wlr.scene_node_raise_to_top(toplevel[0].scene_tree[0].node)
	wl.list_remove(toplevel[0].link)
	wl.list_insert(server[0].toplevels, toplevel[0].link)
	--[[Activate the new surface]]
	wlr.xdg_toplevel_set_activated(toplevel[0].xdg_toplevel, true)
	--[[
	...
	if keyboard ~= nil then
		wlr.seat_keyboard_notify_enter(seat, toplevel[0].xdg_toplevel[0].base[0].surface,
...
mod.keyboard_handle_modifiers = function(listener, data)
	...
	local keyboard = wl.container_of(listener, keyboard, modifiers)
	--[[
	...
	wlr.seat_set_keyboard(keyboard[0].server[0].seat, keyboard[0].wlr_keyboard)
	--[[Send modifiers to the client.]]
	wlr.seat_keyboard_notify_modifiers(keyboard[0].server[0].seat,
...
mod.handle_keybinding = function(server, sym)
	...
	if sym == xkb_key.escape then
		wl.display_terminate(server[0].wl_display)
	elseif sym == xkb_key.f1 then
		--[[Cycle to the next toplevel]]
		if wl.list_length(server[0].toplevels) >= 2 then
			local next_toplevel =
					wl.container_of(server[0].toplevels.prev, next_toplevel, link)
...
mod.keyboard_handle_key = function(listener, data)
	...
	local keyboard =
			wl.container_of(listener, keyboard, key)
	...
	local handled = false
	local modifiers = wlr.keyboard_get_modifiers(keyboard[0].wlr_keyboard)
	if bit.band(modifiers, wlr.modifier.alt) ~= 0 and event[0].state == wl_keyboard_key_state.pressed then
	...
	if not handled then
		wlr.seat_set_keyboard(seat, keyboard[0].wlr_keyboard)
		wlr.seat_keyboard_notify_key(seat, event[0].time_msec,
...
mod.keyboard_handle_destroy = function(listener, data)
	local keyboard = wl.container_of(listener, keyboard, destroy)
	wl.list_remove(keyboard[0].modifiers.link)
	wl.list_remove(keyboard[0].key.link)
	wl.list_remove(keyboard[0].destroy.link)
	wl.list_remove(keyboard[0].link)
	free(keyboard)
end
...
mod.server_new_keyboard = function(server, device)
	local wlr_keyboard = wlr.keyboard_from_input_device(device)
	...
	wlr.keyboard_set_keymap(wlr_keyboard, keymap)
	xkb_keymap_unref(keymap)
	xkb_context_unref(context)
	wlr.keyboard_set_repeat_info(wlr_keyboard, 25, 600)
	keyboard[0].modifiers.notify = mod.keyboard_handle_modifiers
	wl.signal_add(wlr_keyboard[0].events.modifiers, keyboard[0].modifiers)
	keyboard[0].key.notify = mod.keyboard_handle_key
	wl.signal_add(wlr_keyboard[0].events.key, keyboard[0].key)
	keyboard[0].destroy.notify = mod.keyboard_handle_destroy
	wl.signal_add(device[0].events.destroy, keyboard[0].destroy)
	wlr.seat_set_keyboard(server[0].seat, keyboard[0].wlr_keyboard)
	wl.list_insert(server[0].keyboards, keyboard[0].link)
end
...
mod.server_new_pointer = function(server, device)
	 ...
	 * acceleration, etc. ]]
	wlr.cursor_attach_input_device(server[0].cursor, device)
end
...
mod.server_new_input = function(listener, data)
	 ...
	 * available. ]]
	local server = wl.container_of(listener, server, new_input)
	local device = data
	...
	if not wl.list_empty(server[0].keyboards) then
		caps = bit.bor(caps, wl_seat_capability.keyboard)
	end
	wlr.seat_set_capabilities(server[0].seat, caps)
end

--[[@param listener ptr_c<wl_listener>]]
--[[@param data ptr_c<unknown>]]
mod.seat_request_cursor = function(listener, data)
	local server = wl.container_of(
	...
	if focused_client == event[0].seat_client then
		...
		wlr.cursor_set_surface(server[0].cursor, event[0].surface,
...
mod.seat_request_set_selection = function(listener, data)
	...
	local server = wl.container_of(listener, server, request_set_selection)
	local event = data
	wlr.seat_set_selection(server[0].seat, event[0].source, event[0].serial)
end
...
mod.desktop_toplevel_at = function(server, lx, ly,
	...
	local node = wlr.scene_node_at(server[0].scene[0].tree.node, lx, ly, sx, sy)
	if node == nil or node[0].type ~= wlr_scene_node.buffer then
		return nil
	end
	local scene_buffer = wlr.scene_buffer_from_node(node)
	local scene_surface =
			wlr.scene_surface_try_from_buffer(scene_buffer)
...
mod.process_cursor_move = function(server, time)
	...
	wlr.scene_node_set_position(toplevel[0].scene_tree[0].node,
...
mod.process_cursor_resize = function(server, time)
	...
	local wlr_box
	local geo_box
	wlr.xdg_surface_get_geometry(toplevel[0].xdg_toplevel[0].base, geo_box)
	wlr.scene_node_set_position(toplevel[0].scene_tree[0].node,
		new_left - geo_box.x, new_top - geo_box.y)
	local new_width = new_right - new_left
	local new_height = new_bottom - new_top
	wlr.xdg_toplevel_set_size(toplevel[0].xdg_toplevel, new_width, new_height)
end
...
mod.process_cursor_motion = function(server, time)
	...
	if not toplevel then
		wlr.cursor_set_xcursor(server[0].cursor, server[0].cursor_mgr, "default")
	end
	if surface ~= nil then
		wlr.seat_pointer_notify_enter(seat, surface, sx, sy)
		wlr.seat_pointer_notify_motion(seat, time, sx, sy)
	else
		--[[ Clear pointer focus so future button events and such are not sent to
		 * the last client to have the cursor over it. ]]
		wlr.seat_pointer_clear_focus(seat)
	end
end
...
mod.server_cursor_motion = function(listener, data)
	local server = wl.container_of(listener, server, cursor_motion)
	local event = data
	wlr.cursor_move(server[0].cursor, event[0].pointer[0].base,
...
mod.server_cursor_motion_absolute = function(listener, data)
	local server = wl.container_of(listener, server, cursor_motion_absolute)
	local event = data
	wlr.cursor_warp_absolute(server[0].cursor, event[0].pointer[0].base, event[0].x,
		event[0].y)
	mod.process_cursor_motion(server, event[0].time_msec)
end
...
mod.server_cursor_button = function(listener, data)
	local server = wl.container_of(listener, server, cursor_button)
	local event = data
	wlr.seat_pointer_notify_button(server[0].seat,
...
mod.server_cursor_axis = function(listener, data)
	...
	local server = wl.container_of(listener, server, cursor_axis)
	local event = data
	--[[Notify the client with pointer focus of the axis event.]]
	wlr.seat_pointer_notify_axis(server[0].seat,
...
mod.server_cursor_frame = function(listener, data)
	local server = wl.container_of(listener, server, cursor_frame)
	--[[Notify the client with pointer focus of the frame event.]]
	wlr.seat_pointer_notify_frame(server[0].seat)
end
...
mod.output_frame = function(listener, data)
	...
	local output = wl.container_of(listener, output, frame)
	local scene = output[0].server[0].scene

	local scene_output = wlr.scene_get_scene_output(
		scene, output[0].wlr_output)

	--[[Render the scene if needed and commit the output]]
	wlr.scene_output_commit(scene_output, nil)
	local now --[[@type timespec_c]]
	--[[FIXME: call gettime]]
	clock_gettime(CLOCK_MONOTONIC, now)
	wlr.scene_output_send_frame_done(scene_output, now)
end
...
mod.output_request_state = function(listener, data)
	...
	local output = wl.container_of(listener, output, request_state)
	local event = data
	wlr.output_commit_state(output[0].wlr_output, event[0].state)
end

--[[@param listener ptr_c<wl_listener>]]
--[[@param data ptr_c<unknown>]]
mod.output_destroy = function(listener, data)
	local output = wl.container_of(listener, output, destroy)

	wl.list_remove(output[0].frame.link)
	wl.list_remove(output[0].request_state.link)
	wl.list_remove(output[0].destroy.link)
	wl.list_remove(output[0].link)
	free(output)
end
...
mod.server_new_output = function(listener, data)
	local server = wl.container_of(listener, server, new_output)
	local wlr_output = data
	wlr.output_init_render(wlr_output, server[0].allocator, server[0].renderer)
	local state --[[@type wlr_output_state]]
	wlr.output_state_init(state)
	wlr.output_state_set_enabled(state, true)
	local mode = wlr.output_preferred_mode(wlr_output)
	if mode ~= nil then wlr.output_state_set_mode(state, mode) end
	wlr.output_commit_state(wlr_output, state)
	wlr.output_state_finish(state)
	...
	wl.signal_add(wlr_output[0].events.frame, output.frame)
	output[0].request_state.notify = mod.output_request_state
	wl.signal_add(wlr_output[0].events.request_state, output[0].request_state)
	output[0].destroy.notify = mod.output_destroy
	wl.signal_add(wlr_output[0].events.destroy, output[0].destroy)
	wl.list_insert(server[0].outputs, output[0].link)
	local l_output = wlr.output_layout_add_auto(server[0].output_layout, wlr_output)
	local scene_output = wlr.scene_output_create(server[0].scene, wlr_output)
	wlr.scene_output_layout_add_output(server[0].scene_layout, l_output, scene_output)
end

--[[@param listener ptr_c<wl_listener>]]
--[[@param data ptr_c<unknown>]]
mod.xdg_toplevel_map = function(listener, data)
	local toplevel = wl.container_of(listener, toplevel, map)

	wl.list_insert(toplevel[0].server[0].toplevels, toplevel[0].link)
...
mod.xdg_toplevel_unmap = function(listener, data)
	local toplevel = wl.container_of(listener, toplevel, unmap)
...
mod.xdg_toplevel_commit = function(listener, data)
	local toplevel = wl.container_of(listener, toplevel, commit)
	if toplevel[0].xdg_toplevel[0].base[0].initial_commit then
		wlr.xdg_toplevel_set_size(toplevel[0].xdg_toplevel, 0, 0)
	end
end
...
mod.xdg_toplevel_destroy = function(listener, data)
	...
	local toplevel = wl.container_of(listener, toplevel, destroy)
	wl.list_remove(toplevel[0].map.link)
	wl.list_remove(toplevel[0].unmap.link)
	wl.list_remove(toplevel[0].commit.link)
	wl.list_remove(toplevel[0].destroy.link)
	wl.list_remove(toplevel[0].request_move.link)
	wl.list_remove(toplevel[0].request_resize.link)
	wl.list_remove(toplevel[0].request_maximize.link)
	wl.list_remove(toplevel[0].request_fullscreen.link)
	free(toplevel)
end
...
mod.begin_interactive = function(toplevel, mode, edges)
	...
	if toplevel[0].xdg_toplevel[0].base[0].surface ~= wlr.surface_get_root_surface(focused_surface) then
		return
	end
...
mod.xdg_toplevel_request_move = function(listener, data)
	local toplevel = wl.container_of(listener, toplevel, request_move)
...
mod.xdg_toplevel_request_resize = function(listener, data)
	local event = data
	local toplevel = wl.container_of(listener, toplevel, request_resize)
...
mod.xdg_toplevel_request_maximize = function(listener, data)
	local toplevel = wl.container_of(listener, toplevel, request_maximize)
	if toplevel[0].xdg_toplevel[0].base[0].initialized then
		wlr.xdg_surface_schedule_configure(toplevel[0].xdg_toplevel[0].base)
	end
end
...
mod.xdg_toplevel_request_fullscreen = function(listener, data)
	...
	local toplevel = wl.container_of(listener, toplevel, request_fullscreen)
	if toplevel[0].xdg_toplevel[0].base[0].initialized then
		wlr.xdg_surface_schedule_configure(toplevel[0].xdg_toplevel[0].base)
	end
end
...
mod.server_new_xdg_toplevel = function(listener, data)
	local server = wl.container_of(listener, server, new_xdg_toplevel)
	...
	toplevel[0].scene_tree = wlr.scene_xdg_surface_create(toplevel[0].server[0].scene[0].tree, xdg_toplevel[0].base)
	...
	wl.signal_add(xdg_toplevel[0].base[0].surface[0].events.map, toplevel[0].map)
	toplevel[0].unmap.notify = mod.xdg_toplevel_unmap
	wl.signal_add(xdg_toplevel[0].base[0].surface[0].events.unmap, toplevel[0].unmap)
	toplevel[0].commit.notify = mod.xdg_toplevel_commit
	wl.signal_add(xdg_toplevel[0].base[0].surface[0].events.commit, toplevel[0].commit)
	toplevel[0].destroy.notify = mod.xdg_toplevel_destroy
	wl.signal_add(xdg_toplevel[0].events.destroy, toplevel[0].destroy)
	--[[cotd]]
	toplevel[0].request_move.notify = mod.xdg_toplevel_request_move
	wl.signal_add(xdg_toplevel[0].events.request_move, toplevel[0].request_move)
	toplevel[0].request_resize.notify = mod.xdg_toplevel_request_resize
	wl.signal_add(xdg_toplevel[0].events.request_resize, toplevel[0].request_resize)
	toplevel[0].request_maximize.notify = mod.xdg_toplevel_request_maximize
	wl.signal_add(xdg_toplevel[0].events.request_maximize, toplevel[0].request_maximize)
	toplevel[0].request_fullscreen.notify = mod.xdg_toplevel_request_fullscreen
	wl.signal_add(xdg_toplevel[0].events.request_fullscreen, toplevel[0].request_fullscreen)
end

--[[@param listener ptr_c<wl_listener>]]
--[[@param data ptr_c<unknown>]]
mod.xdg_popup_commit = function(listener, data)
	local popup = wl.container_of(listener, popup, commit)
	if popup[0].xdg_popup[0].base[0].initial_commit then
		wlr.xdg_surface_schedule_configure(popup[0].xdg_popup[0].base)
	end
end
...
mod.xdg_popup_destroy = function(listener, data)
	local popup = wl.container_of(listener, popup, destroy)
	wl.list_remove(popup[0].commit.link)
	wl.list_remove(popup[0].destroy.link)
	free(popup)
end
...
mod.server_new_xdg_popup = function(listener, data)
	...
	local parent = wlr.xdg_surface_try_from_wlr_surface(xdg_popup[0].parent)
	assert(parent ~= nil)
	local parent_tree = parent[0].data
	xdg_popup[0].base[0].data = wlr.scene_xdg_surface_create(parent_tree, xdg_popup[0].base)
	popup[0].commit.notify = mod.xdg_popup_commit
	wl.signal_add(xdg_popup[0].base[0].surface[0].events.commit, popup[0].commit)
	popup[0].destroy.notify = mod.xdg_popup_destroy
	wl.signal_add(xdg_popup[0].events.destroy, popup[0].destroy)
end

mod.run = function()
	wlr.log_init(WLR_DEBUG, nil)
	local server = ffi.new("tinywl_server") --[[@type tinywl_server]]
	server.wl_display = wl.display_create()
	server.backend = wlr.backend_autocreate(wl.display_get_event_loop(server.wl_display), nil)
	if server.backend == nil then
		wlr.log(WLR_ERROR, "failed to create wlr_backend")
		return 1
	end
	server.renderer = wlr.renderer_autocreate(server.backend)
	if server.renderer == nil then
		wlr.log(WLR_ERROR, "failed to create wlr_renderer")
		return 1
	end
	wlr.renderer_init_wl_display(server.renderer, server.wl_display)
	server.allocator = wlr.allocator_autocreate(server.backend, server.renderer)
	if server.allocator == nil then
		wlr.log(WLR_ERROR, "failed to create wlr_allocator")
		return 1
	end
	wlr.compositor_create(server.wl_display, 5, server.renderer)
	wlr.subcompositor_create(server.wl_display)
	wlr.data_device_manager_create(server.wl_display)
	server.output_layout = wlr.output_layout_create(server.wl_display)
	wl.list_init(server.outputs)
	server.new_output.notify = mod.server_new_output
	wl.signal_add(server.backend[0].events.new_output, server.new_output)
	server.scene = wlr.scene_create()
	server.scene_layout = wlr.scene_attach_output_layout(server.scene, server.output_layout)
	wl.list_init(server.toplevels)
	server.xdg_shell = wlr.xdg_shell_create(server.wl_display, 3)
	server.new_xdg_toplevel[0].notify = mod.server_new_xdg_toplevel
	wl.signal_add(server.xdg_shell[0].events.new_toplevel, server.new_xdg_toplevel)
	server.new_xdg_popup.notify = mod.server_new_xdg_popup
	wl.signal_add(server.xdg_shell[0].events.new_popup, server.new_xdg_popup)
	server.cursor = wlr.cursor_create()
	wlr.cursor_attach_output_layout(server.cursor, server.output_layout)
	server.cursor_mgr = wlr.xcursor_manager_create(nil, 24)
	server.cursor_mode = tinywl_cursor_mode.passthrough
	server.cursor_motion.notify = mod.server_cursor_motion
	wl.signal_add(server.cursor[0].events.motion, server.cursor_motion)
	server.cursor_motion_absolute.notify = mod.server_cursor_motion_absolute
	wl.signal_add(server.cursor[0].events.motion_absolute,
		server.cursor_motion_absolute)
	server.cursor_button.notify = mod.server_cursor_button
	wl.signal_add(server.cursor[0].events.button, server.cursor_button)
	server.cursor_axis.notify = mod.server_cursor_axis
	wl.signal_add(server.cursor[0].events.axis, server.cursor_axis)
	server.cursor_frame.notify = mod.server_cursor_frame
	wl.signal_add(server.cursor[0].events.frame, server.cursor_frame)
	wl.list_init(server.keyboards)
	server.new_input.notify = mod.server_new_input
	wl.signal_add(server.backend[0].events.new_input, server.new_input)
	server.seat = wlr.seat_create(server.wl_display, "seat0")
	server.request_cursor.notify = mod.seat_request_cursor
	wl.signal_add(server.seat[0].events.request_set_cursor,
		server.request_cursor)
	server.request_set_selection.notify = mod.seat_request_set_selection
	wl.signal_add(server.seat[0].events.request_set_selection,
		server.request_set_selection)
	local socket = wl.display_add_socket_auto(server.wl_display)
	if not socket then
		wlr.backend_destroy(server.backend)
		return 1
	end
	if not wlr.backend_start(server.backend) then
		wlr.backend_destroy(server.backend)
		wl.display_destroy(server.wl_display)
		return 1
	end
	...
	-- end
	wlr.log(WLR_INFO, "Running Wayland compositor on WAYLAND_DISPLAY=%s", socket)
	wl.display_run(server.wl_display)
	wl.display_destroy_clients(server.wl_display)
	wlr.scene_node_destroy(server.scene[0].tree.node)
	wlr.xcursor_manager_destroy(server.cursor_mgr)
	wlr.cursor_destroy(server.cursor)
	wlr.allocator_destroy(server.allocator)
	wlr.renderer_destroy(server.renderer)
	wlr.backend_destroy(server.backend)
	wl.display_destroy(server.wl_display)
	return 0
end
